...
# make paths
proj.path <- here("/home/genomics/icornelis/02_ZEROimpact/01_12S/NJ2021/MiFish-UE_run2",paste0("MiFish_UE-S_concatenated"))
...
#trackfilter <- cbind(out.sense, out.antisense)
...
seqtab.nochim2<-seqtab.nochim[!rowSums(seqtab.nochim)==0,]

#Create rarecurve before concatenation of PCR replicates
raremax = 10000
rarecurve(seqtab.nochim, ylab = "ASVs", main = "Rarecurve of unconcatenated samples before taxonomic assignment", label = FALSE, step =1000) #20000 is situated just after the steep part of the graph
abline(v = raremax)
Notrarefied <- c(rownames(seqtab.nochim[rowSums(seqtab.nochim)<raremax,]))
...
seqtab.concat$names <- str_sub(rownames(seqtab.concat), end=-4) # removes PCR number (_S1, _S2 or _S3) from samplenames
...
raremax <- 10000
rarecurve(seqtab.concatenated, ylab = "ASVs", main = "Rarecurve of concatenated samples before taxonomic assignment", label = FALSE, step =1000)
abline(v = raremax)
...
write_tsv(as_tibble(seqtab.nochim2.rarefied, rownames="asv"), file=paste0(proj.path,"/results/seqtab.concatenated.unrarefied"))
...
taxa <- assignTaxonomy(merged.seqtab.nochim, "/home/genomics/icornelis/03_RawScripts/Step4_12S_ReferenceDB_TaxonomicAssignment/12S_references_20220906.fas", minBoot = 80, multithread = TRUE)
...
table_rarefied.concatenated <- as.data.frame(t(seqtab.concatenated.rarefied))
...
table_rarefied.concatenated$ASV<-otus.df$names
...
freq_table_rare$ASV<-otus.df$names
write_tsv(as_tibble(freq_table_rare), file=paste0(proj.path,"/results/frequency_table_rare.tsv"))

##Vanaf hier opsplitsen naar nieuw Script voor data verwerking

#remove negative controls
env <- read.csv("/home/genomics/icornelis/02_ZEROimpact/01_12S/NJ2021/MiFish-UE_run2/Step3_DADA2/environmental_data.csv",  header=TRUE, sep=";")
...
table_rarefied_assigned <- table_rarefied.concatenated[!table_rarefied.concatenated$Order %in% c(NA),]
merged.seqtab.nochim.assigned <- as.data.frame(t(table_rarefied_assigned[,1:(ncol(table_rarefied_assigned)-8)]))
table_rarefied_assigned2 <- table_rarefied.concatenated2[!table_rarefied.concatenated2$Order %in% c(NA),]
...
keep_samples <- c(rownames(seqtab.concatenated))
env<-env[env$Niskin.sample %in% keep_samples,]
keep_samples_rarefired <- c(rownames(seqtab.concatenated.rarefied2))
env_rarefied <- env[env$Niskin.sample %in% keep_samples_rarefired,]
...
seqtab.concatenated.rarefied <- seqtab.concatenated.rarefied[env$Niskin.sample,]

#rarecurve with color
rarecurve(seqtab.concatenated.rarefied, ylab = "ASVs", main = "Rarecurve of rarefied samples before taxonomic assignment", label=FALSE, col = as.vector(env$Environment_color), step = 1000)
rarecurve(merged.seqtab.nochim.assigned, ylab = "Assigned ASVs", main = "Rarecurve of rarefied samples after taxonomic assignment", col = as.vector(env$Environment_color), label = FALSE, step =1000)
rarecurve(merged.seqtab.nochim.assigned2, ylab = "Assigned ASVs", main = "Rarecurve of rarefied samples after taxonomic assignment", col = as.vector(env_rarefied$Environment_color), label = FALSE, step =1000)
...
rarecurve(seqtab.concatenated, ylab = "ASVs", main = "Rarecurve of concatenated samples before taxonomic assignment", col = as.vector(env$Environment_color), label = FALSE, step =1000)
abline(v = raremax)
