...
#THRESHOLD 0.5 prevalence and field negatives removed
...
contam_taxa[is.na(contam_taxa)] <- "NA" #125 taxa
fish_classes <- readRDS(file = paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/REnvironment/Fish_classes.rds"))
contam_taxa_fish <- as.data.frame(contam_taxa[contam_taxa$Class %in% fish_classes,])
...
write.xlsx(as.data.frame(contam_taxa),
           paste0(proj.path, "/MiFish_UE-S_concatenated/results_v2/FullContaminationTable_raw_DecontamPrevalence_NoFieldNeg_Threshold05.xlsx"), sheetName= "Contamination Table", colNames = TRUE, rowNames = TRUE, append = FALSE)

### 4) calculate decontam with prevalence method and the field negatives excluded
contamination_table_raw_prev_2 <- isContaminant(phylo_raw, method = "prevalence", 
                                              neg = "is.neg", 
                                              threshold = 0.4, #change the treshold to find the best division between true ASVs and contaminant ASVs
                                              batch="Batch",
                                              normalize = TRUE, # default TRUE, if FALSE data should be counts from equal-depth samples
                                              detailed = TRUE) # if TRUE the return value is a data.frame

# plot histogram and prevalence of ASVs in positive vs negative samples
hist(contamination_table_raw_prev_2$p.prev, main="Histogram Prevalence Treshold 0.4 With Field negatives", xlab="p.prev values")
...
#contaminants assigned to fish taxa
contam_taxa_Field[is.na(contam_taxa_Field)] <- "NA" #112 taxa
contam_taxa_fish_Field <- as.data.frame(contam_taxa_Field[contam_taxa_Field$Class %in% fish_classes,])

#The prevalence method with a threshold of 0.4 was without the Field negatives included was chosen based on the plots 
write.xlsx(as.data.frame(contam_taxa_Field),
           paste0(proj.path, "/MiFish_UE-S_concatenated/results_v2/FullContaminationTable_raw_DecontamPrevalence_WithFieldNeg_Threshold04.xlsx"), sheetName= "Contamination Table", colNames = TRUE, rowNames = TRUE, append = FALSE)
...
ASV_contaminants <- rownames(contamination_table_raw_prev[which(contamination_table_raw_prev$contaminant == TRUE),])
...
saveRDS(env_unrarefied, file = paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/REnvironment/env_unrarefied.rds"))
...
#ASVs assigned to fish taxa
freshwater_fish <- readRDS(file = paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/REnvironment/Fish_Freshwater.rds"))
table_unrarefied_concatenated_fish <- as.data.frame(table_unrarefied_concatenated_clean[table_unrarefied_concatenated_clean$Class %in% fish_classes,])
table_unrarefied_concatenated_fish <- as.data.frame(table_unrarefied_concatenated_fish[!table_unrarefied_concatenated_fish$Species %in% c(freshwater_fish, "NA"),])

#remove samples from environmental data that are now absent from the data3
seqtab_fish <- as.data.frame(t(table_unrarefied_concatenated_fish[,1:(ncol(table_unrarefied_concatenated_fish)-11)]))
#Put samples in same order as environmental data
seqtab_fish <- seqtab_fish[env_unrarefied$Niskin.sample,] #to match the colors correctly the samples in seqtab.concatenated must be in the same order as your environmental data
rarecurve(seqtab_fish, ylab = "Fish ASVs", main = "Rarecurve of unrarefied samples after taxonomic assignment", col = as.vector(env_unrarefied$Environment_color), label = FALSE, step =100)
abline(v = raremax)
...
write.xlsx(as.data.frame(table_unrarefied_concatenated), 
           paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/table_unrarefied_AllASVs_FullTaxonomicAssignment.xlsx"), 
           sheetName = "FullTaxAss_AllASVs", colNames = TRUE, rowNames = FALSE, append = FALSE)
write.xlsx(as.data.frame(table_raw_clean), 
           paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/table_raw_CleanedASVs_NoField_FullTaxonomicAssignment.xlsx"), 
...
write.xlsx(as.data.frame(table_unrarefied_concatenated_clean), 
           paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/table_unrarefied_concatenated_ASVs_NoField_FullTaxonomicAssignment.xlsx"), 
...
write.xlsx(as.data.frame(table_rarefied_clean_allSamples), 
           paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/table_rarefied_CleanedASVs_NoField_FullTaxonomicAssignment_allSamples.xlsx"), 
...
write.xlsx(as.data.frame(table_rarefied_clean), 
           paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/table_rarefied_CleanedASVs_NoField_FullTaxonomicAssignment.xlsx"), 
...
#create the rarecurve of fish
#ASVs assigned to fish taxa
#table_rarefied_cleaned[is.na(table_rarefied_cleaned)] <- "NA"
table_rarefied_clean_fish <- as.data.frame(table_rarefied_clean[table_rarefied_clean$Class %in% fish_classes,])
seqtab_rarefied_clean_fish <- as.data.frame(t(table_rarefied_clean_fish[,1:(ncol(table_rarefied_clean_fish)-11)]))
rarecurve(seqtab_rarefied_clean_fish, ylab = "Fish ASVs", 
          main = "Rarecurve of Rarefied samples after taxonomic assignment", 
          col = as.vector(env_rarefied$Environment_color), label = FALSE, step =100)

#Delete ASVs with 10% in neg control
neg_ctrl <- table_raw[, grepl("neg", colnames(table_raw))]
...
for(i in 1:ncol(neg_ctrl)){relative_abundance[,i] <- (neg_ctrl[,i]/rowSums(table_raw[1:(ncol(table_raw)-11)]))*100}
rownames(relative_abundance) <- table_raw$ASV
keep_ASVs <- c(rownames(relative_abundance[!rowSums(relative_abundance > 10),]))
table_unrarefied_clean <- table_raw[table_raw$ASV %in% keep_ASVs,]
table_unrarefied_concatenated_clean <- table_unrarefied_concatenated[table_unrarefied_concatenated$ASV %in% keep_ASVs,]
table_unrarefied_contaminants <- table_raw[!table_raw$ASV %in% keep_ASVs,]
write.xlsx(as.data.frame(table_unrarefied_clean), paste0(proj.path,"/MiFish_UE-S_concatenated/results/table_unrarefied_FullTaxonomicAssignment_clean.xlsx"))
write.xlsx(as.data.frame(table_unrarefied_concatenated_clean), paste0(proj.path,"/MiFish_UE-S_concatenated/results_v2/table_unrarefied_concatenated_FullTaxonomicAssignment_clean.xlsx"))
write.xlsx(as.data.frame(table_unrarefied_contaminants), paste0(proj.path,"/MiFish_UE-S_concatenated/results/table_unrarefied_FullTaxonomicAssignment_contaminants.xlsx"))